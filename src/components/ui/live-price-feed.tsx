...
export function LivePriceFeed({
  ...
  const [updateAtTick, setUpdateAtTick] = useState(new Date());

  useInterval(() => setUpdateAtTick(new Date()), 1000);

  return (
    <div
      className={cn(
        'flex flex-col gap-4 rounded-lg bg-white p-5 dark:bg-light-dark',
        {
    ...
    >
      ...
      <div
        ...
        style={{ width: 'auto' }}
      >
        Updated At: {updateAtTick.toLocaleString()}
      </div>
    </div>
  );
}
